// Simple token storage using localStorage + presence cookie for middleware
...
export function setToken(token: string) {
  ...
  try {
    // lightweight presence cookie so middleware can guard routes
...
export function isLoggedIn(): boolean {
  // Consider logged in only when both token and the presence cookie exist
